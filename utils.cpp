...
int main() {
    ...
    model.addLayer(std::make_shared<ConvolutionLayer>(1, 28, 28, 3, 8, 1)); // in_c, in_h, in_w, k_size, num_filters, stride
    ...
    model.addLayer(std::make_shared<ConvolutionLayer>(8, 14, 14, 3, 16, 1));
    model.addLayer(std::make_shared<ActivationFunction>(ActivationFunction::Type::ReLU));
    model.addLayer(std::make_shared<PoolingLayer>(2, 2, 2, PoolingLayer::Type::MAX));

    // Flatten Layer
    model.addLayer(std::make_shared<FlattenLayer>(16, 5, 5)); // assuming after pooling: 5x5 per channel
